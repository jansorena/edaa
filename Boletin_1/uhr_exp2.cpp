...
#include "src/sequential_search.cpp"
#include "src/binary_search.cpp"
#include "src/galloping_search.cpp"
...
int main(int argc, char *argv[])
{
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        int64_t fixed_n = 20000;
        std::vector<int> data(fixed_n);
        for (int i = 0; i < fixed_n; i++) {
            data[i] = i; 
        }

        int target = n - 1; // variable target
        ...
        for (i = 0; i < runs; i++) {
            ...
            //sequential_search(data.data(), fixed_n, target); //results_sequential
            //binary_search(data.data(), fixed_n, target); //results binary
            galloping_search(data.data(), fixed_n, target); //results_galloping
