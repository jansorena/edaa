...
int main(int argc, char *argv[])
{
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        for (i = 0; i < runs; i++) {
            ...
            //sequential_search(data.data(), n, target); //results_sequential
            //binary_search(data.data(), n, target); //results binary
            galloping_search(data.data(), n, target); //results_galloping
