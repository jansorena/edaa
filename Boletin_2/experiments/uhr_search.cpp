...
int main(int argc, char *argv[])
{
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        //std::uniform_int_distribution<std::int64_t> u_distr(0, n);

        //std::set<std::int64_t> rb_tree;
        //AVLTree<std::int64_t> avl_tree;
        SplayTree sp_tree;
        //skiplist<std::int64_t> sl;
        ...
        // pair vector
        std::vector<std::pair<std::int64_t, std::int64_t>> freq_vector(key_frequency.begin(), key_frequency.end());

        // sort
        std::sort(freq_vector.begin(), freq_vector.end(), [](const std::pair<std::int64_t, std::int64_t>& a, const std::pair<std::int64_t, std::int64_t>& b) {
        ...
        std::size_t top_n = n / 2; // Ajusta según cuántas claves frecuentes deseas incluir
        ...
        //std::uniform_int_distribution<std::int64_t> freq_distr(0, most_frequent_keys.size());
        std::uniform_int_distribution<std::int64_t> freq_distr(0, least_frequent_keys.size());
        //*****************************
        ...
        for (i = 0; i < runs; i++) {
            ...
            std::int64_t same_key = least_frequent_keys[freq_distr(rng)];

            // Function to test goes here
            for(std::int64_t j = 0; j < n; j++){
                //std::int64_t value = u_distr(rng);
                //std::int64_t frequent_key = most_frequent_keys[freq_distr(rng)];
                //std::int64_t not_frequent_key = least_frequent_keys[freq_distr(rng)];
                //avl_tree.find(value);
                //rb_tree.find(value);
                //sp_tree.find(frequent_key);
                sp_tree.find(same_key);
                //sl.find(value);
            }
