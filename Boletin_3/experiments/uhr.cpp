...
int main(int argc, char *argv[])
{
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        std::vector<std::string> text_filenames = {"dna.50MB", "dna.100MB", "dna.200MB", "dna.385MB"};
        std::string text = load_text(text_filenames[n-1]);
        std::size_t pattern_size = 10;
        std::string pattern = generate_random_pattern(pattern_size, rng);

        suffix_array sa(text);
        std::cout << "Text name: " << text_filenames[n-1] << std::endl;
        std::cout << "Text size: " << text.size() << std::endl;
        std::cout << "Suffix array space in bytes: " << sa.memory_usage() << std::endl;
        std::cout << "Suffix array space in MB: " << sa.memory_usage() / (1024.0 * 1024.0) << std::endl;

        KMP kmp(pattern);
        std::cout << "KMP space in bytes: " << kmp.memory_usage() << std::endl;
        std::cout << "KMP space in MB: " << kmp.memory_usage() / (1024.0 * 1024.0) << std::endl;
