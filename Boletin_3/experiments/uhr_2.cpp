...
std::string generarPatronAleatorio(const std::string& texto, size_t longitudPatron) {
    if (longitudPatron > texto.size()) {
        throw std::invalid_argument("La longitud del patrón no puede ser mayor que la longitud del texto.");
    }

    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, texto.size() - 1);

    std::string patron;
    for (size_t i = 0; i < longitudPatron; ++i) {
        patron += texto[dis(gen)];
    }
...
int main(int argc, char *argv[])
{
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        // Test configuration goes here
        std::string pattern = generarPatronAleatorio(text, n);
