...
int main(int argc, char *argv[])
{
    ...
    std::uniform_int_distribution<std::int64_t> u_distr(1, 1000000); // change depending on app
    ...
    for (n = lower; n <= upper; n += step) {
        ...
        // Get random index for query
        std::uniform_int_distribution<std::int64_t> u_distr_query(0, n - 2);
        std::int64_t i = u_distr_query(rng);
        std::int64_t j = u_distr_query(rng);
        ...
        sparse_table<std::int64_t> ST(A, f);
